...
function Frameworks() {
    let uniqueKey = 1000;
    const renderFrameworks = frontEndFrameworks.map(item => <li key={uniqueKey += 1}>{item}</li>); // change code here
    return (
        <div>
...
class MyComponent extends React.Component {
    ...
    render() {
        ...
        const renderOnline = usersOnline.map(user => <li key={keyNum += 1}>{user.username}</li>);
        return (
            <div>
